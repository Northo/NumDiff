...
\pgfplotsinvokeforeach {1,2,3,4} {
	\ifthenelse{\equal{#1}{1}}{
		\nextgroupplot[ylabel={$\Ltwoerror{u-U}/\Ltwoerror{u}$},title={$f(x) = -2$}];
...
\end{groupplot}
\end{tikzpicture}
\caption{\label{femconvergence}Convergence plots for four problems that compare uniform mesh refinement with the two adaptive mesh refinement strategies. (TODO: show order?)}
\end{figure}
