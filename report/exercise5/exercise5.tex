\section{Poisson equation in one dimension using finite element method}
...
We use Gauss-Legendre quadrature to do these integrals.
...
\figureseries{exercise5/data/UMR}{femumr}{Uniform mesh refinement on four problems with the average error strategy, showing the evolution of an initial mesh whose elements are split in half over three iterations, along with the distribution of $L_2$-errors across the elements.}
...
\figureseries{exercise5/data/AMR}{femamr}{Adaptive mesh refinement on four problems with the average strategy, showing the evolution of an initial uniform mesh as it is refined over three iterations. Elements whose $L_2$-error lie above the reference error \protect\begin{tikzpicture}[baseline=-0.5ex]\protect\draw[black, dashed] (0,0.0) -- (4ex,0.0);\protect\end{tikzpicture} are split in half.}
...
\begin{groupplot}[
	...
	\nextgroupplot[xlabel=$M+1$, ylabel={$\Ltwoerror{u-U}/\Ltwoerror{u}$},title={$f(x) = -2$},xmin=0.5,xmax=4000,ymin=1e-8,ymax=5e-1,
	...
	\addplot [domain=0.1:4000, dashed, samples=2, black, forget plot] {0.19/x^2} node [pos=0.3,pin={0:$\Oh(h^2)$}] {};
	...
	\addplot table [x=M3, y=E3] {exercise5/data/convergence/f1-convergence.dat};

	\nextgroupplot[xlabel=$M+1$, title={$f(x) = -(40000x^2 - 200) \exp(-100x^2)$},xmin=0.5,xmax=4000,ymin=2e-6,ymax=4e-0];
	\addplot [domain=0.1:4000, dashed, samples=2, black, forget plot] {22/x^2} node [pos=0.4,pin={0:$\Oh(h^2)$}] {};
	...
	\addplot table [x=M3, y=E3] {exercise5/data/convergence/f2-convergence.dat};

	\nextgroupplot[xlabel=$M+1$, ylabel={$\Ltwoerror{u-U}/\Ltwoerror{u}$}, title={$f(x) = -(4000000x^2 - 2000) \exp(-1000x^2)$},xmin=0.5,xmax=4000,ymin=1e-5,ymax=4e-0];
	\addplot [domain=0.1:4000, dashed, samples=2, black, forget plot] {122/x^2} node [pos=0.5,pin={0:$\Oh(h^2)$}] {};
	...
	\addplot table [x=M3, y=E3] {exercise5/data/convergence/f3-convergence.dat};

	\nextgroupplot[xlabel=$M+1$, title={$f(x) = 2x^{-4/3}/9$},xmin=0.5,xmax=4000,ymin=5e-6,ymax=0.1];
	\addplot [domain=0.1:4000, dashed, samples=2, black, forget plot] {0.10/x^1.15} node [pos=0.4,pin={0:$\Oh(h^{1.15})$}] {};
	\addplot [domain=0.1:4000, dashed, samples=2, black, forget plot] {0.15/x^2} node [pos=0.65,pin={180:$\Oh(h^{2})$}] {};
	...
	\addplot table [x=M3, y=E3] {exercise5/data/convergence/f4-convergence.dat};

\end{groupplot}
\end{tikzpicture}
\caption{\label{femconvergence}Convergence plots for four problems that compare uniform mesh refinement with the two adaptive mesh refinement strategies.}
\end{figure}
