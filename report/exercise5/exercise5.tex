...
Same as in \cref{poisson_analytical_solution}, but with $f(x) \rightarrow -f(x)$, so that
\begin{equation}
...
We now test our finite element method on four problems, shown in \cref{femumr}, with uniformly sized elements.
In all but the first problem, errors are distributed non-evenly across the elements.
By using uniform elements, we therefore waste a lot of space by using many elements in regions where the solution is already well known.

\figureseries{exercise5/data/UMR}{femumr}{Uniform refinement}

\subsubsection{Adaptive refinement}

This is the motivation for doing adaptive refinement of the elements.
This time, we will do it a little differently than in ONE.
Instead of splitting only one element between each iteration of the numerical solution, we now present two strategies that splits a dynamic number of elements so that all errors reduce below some given level in every iteration.
\begin{enumerate}
...
The prefactor ensures that intervals are split also when all errors are equal up to machine precision, so the refinement does not halt.
...
In \cref{femamr}, we show how the errors distribute on the same problems using the average error strategy.
Finally, in \cref{femconvergence}, we compare the convergence of uniform refinement and the two adaptive refinement strategies.
Note that the adaptive refinement always yields a better result, unlike in ONE!
