...
\begin{equation*}
    \frac{y^{n+1} - y^n}{\Delta t} = \left((1-\theta)g(y^n, t_n)+\theta g(y^{n+1}, t_{n+1})\right), 
\end{equation*}
where $\Delta t$ is the time step and the value of $\theta$ determines the specific numerical scheme 
\begin{align*}
...
\begin{equation}
    ...
    \label{eq:2a}
\end{equation}
The computed solutions for $t \in [0, 0.3]$ is plotted in figure \ref{fig:task2-surface}, 
...
and for our difference formula, the heat equation theta method, we get 
\begin{align*}
...
\begin{align*}
    \tau_m^n = \Oh(k+h^2) \quad for \theta\neq\frac{1}{2},\\
    \tau_m^n = \Oh(k^2+h^2) \quad for \theta = \frac{1}{2}.
\end{align*}
...
whilst the Forward Euler method is only stable if $r=k/h^2 < 1/2$, 
which would impose impractical restrictions on the grids and the refinement.\cite{TODO}
...
and we have plotted the numerical solution in \ref{fig:task2-surface}. 
