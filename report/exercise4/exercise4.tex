...
\begin{groupplot}[
	...
	\pgfplotsinvokeforeach{0,1,...,4} {
		...
		\addplot [black,line width=3pt,domain=-1:+1,samples=100] {sin(deg(pi*(x-#1/4)))};
		\addplot [red,line width=1pt] table [x expr=-1+2*\coordindex/800, y expr=\thisrowno{#1},skip first n=1] {exercise4/timeevol_sin.dat};
	};
\end{groupplot}
...
\begin{figure}
\begin{tikzpicture}
\begin{axis}[ymin=-1.2, ymax=+1.2, no markers,title={Forward Euler ($N=300000$)},ylabel={$U(x,1)$}]
\pgfplotsinvokeforeach{5,15,25} {
	\addplot table {exercise4/snapshot-forward-euler-M#1-N300000.dat};
	\addlegendentry{$M=#1$};
}
\addplot [xlabel=$x$, color=black, domain=-1:+1, very thick] {sin(deg(pi*(x-1)))} node [pos=0.5,pin={0:$u(x,1)$},inner sep=0pt] {};
\end{axis}
\end{tikzpicture}
\begin{tikzpicture}
\begin{axis}[ymin=-1.2, ymax=+1.2, no markers,title={Crank-Nicholson ($N=10$)},yticklabels={,,}]
...
\centering
\begin{tikzpicture}
\begin{loglogaxis}[xlabel=$M$,ylabel={$\Ltwoerror{u-U}/\Ltwoerror{u}$},legend pos=north west,width=8cm,title=Forward Euler,xtick=data,xticklabels={5,10,15,20,25,30}]
...
\end{tikzpicture}
\begin{tikzpicture}
\begin{loglogaxis}[xlabel=$M$,legend pos=north east,width=8cm,title=Crank-Nicholson,xmin=5,xmax=2000]
\addplot [dashed, domain=1:3000, samples=2, color=magenta,forget plot] {40/x^1} node [pos=0.4,pin={0:$\Oh(h^1)$},inner sep=0pt] {};
...
\end{loglogaxis}
\end{tikzpicture}
\caption{\label{ex4_euler_vs_crank_convergence}Convergence plots of the numerical solution $U(x,t)$ for the Forward Euler and Crank-Nicholson method on the problem defined by the exact solution $u(x,t) = \sin(\pi(x-t))$.}
\end{figure}
...
\begin{groupplot}[
	...
	\pgfplotsinvokeforeach{0,1,...,11} {
		...
		\addplot [black] table [x expr=-1+2*\coordindex/800, y expr=\thisrowno{#1}] {exercise4/timeevol_exp.dat};
...
\begin{groupplot}[
	...
	\nextgroupplot[title={Crank-Nicholson ($M=800$)}, xlabel=$t$,yticklabels={,,},cycle list={{red,line width=6pt},{blue,line width=4pt},{green,line width=2pt}},
