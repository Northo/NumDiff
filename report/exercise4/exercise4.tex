...
To save both memory and time, we represent them with sparse matrices.
In addition, to efficiently solve the same system with different right hand sides many times, we $LU$-factorize the sparse matrix for $I - \theta k A$.
...
\begin{groupplot}[
	...
	\addplot [forget plot, black, loosely dashed, samples=2, domain=-1:2] {0.63} node [pos=0.31, pin={280:{$\Ltwoerror{u(x,t)}$}}] {};
	...
	\draw [green] (0.48, 0.95) node {$M=20$};
