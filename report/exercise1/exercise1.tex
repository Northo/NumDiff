...
We find the expected convergence rate by inserting the exact solution into the finite difference formula \eqref{eq:task1-finite-diff-approx}. 
...
\begin{align*}
    \tau_m & = \frac{1}{h^2}\left(-2u_m + 2u_m + 2\frac{h^2}{2}\partial_x^2 u_m + 2\frac{h^4}{24}\partial_x^4 u_m + \Oh(h^6) \right) - f_m\\
           ...
           & = \Oh(h^2), 
\end{align*} 
and the convergence is therefore expected to be of second order. 
...
\caption{
    ...
    Note that a cross inside a circle means that the discrete and continous error measurements agree.
}
\end{figure}
