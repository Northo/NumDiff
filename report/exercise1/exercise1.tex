...
\begin{equation*}
\left| \nonuniformstencil{u}{m}{m+1/2}{m+1} - f(x_m) \right|, \qquad \text{TODO fix interval indexes (m+1/2)}
\end{equation*}
...
This splitting strategy requires neither knowlege of the exact solution or the numerical solution, only on the given source function $f(x)$, which is the most realistic scenario.
\end{enumerate}
...
\caption{
	...
	During adaptive mesh refinement (AMR) with the error strategy, the interval with the largest error $\int \dif x |u(x) - U(x)|$ is split in half.
