...
\begin{tikzpicture}
\begin{groupplot}[
	group style={group size=3 by 4,horizontal sep=0.3cm,vertical sep=0.3cm},
...
\caption{
	...
	During adaptive mesh refinement (AMR) with the error strategy, the interval with the largest error $\int |u(x) - U(x)|$ is split in half.
	Here, $u(x) = \exp(-(x-1/2)^2/0.1)$ is the symmetric solution to $u_{xx} = f(x)$.
	Symmetry is imposed numerically by adding the point $1 - x$ to the grid whenever $x$ is added, unless $x = 1/2$.
}
\end{figure}
