...
\begin{enumerate}
\item Start with a coarse grid with uniform spacing, such as $x_0 = 0$ and $x_1 = 0$ only.
...
This splitting strategy requires neither knowlege of the exact solution or the numerical solution, only on the given source function $f(x)$, which is far more likely than the others to be known in actual applications.
\end{enumerate}
